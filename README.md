...
### Компоненты:
* **MCU:** ESP32 (ESP32-WROOM-32 или готовые платы типа TTGO LoRa32).
* **Radio:** Трансиверы LoRa SX1278 (433 MHz) или SX1276 (868 MHz) — обеспечивают связь вне прямой видимости.
* **Power:** Кастомная система питания на базе **Li-Ion аккумулятора (формат 18650)** с контроллером заряда (BMS/TP4056).
* **Display:** OLED 0.96" (опционально) для мониторинга статуса узла и отладки.
