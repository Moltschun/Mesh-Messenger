...
* **Radio:** Трансиверы LoRa SX1278 (433 MHz) — обеспечивают связь вне прямой видимости.
* **Power:** Кастомная система питания на базе **Li-Ion аккумулятора (формат 14500)** с контроллером заряда (TP4056).
...
* **Autonomy:** Использование емких аккумуляторов 14500 в сочетании с низким потреблением LoRa обеспечивает длительную автономную работу узлов и ретрансляторов.

---
## Энергопотребление и Автономность

Устройство спроектировано для длительной автономной работы в полевых условиях.
Питание осуществляется от компактной сборки Li-Ion 2S (формат 14500).

### Конфигурация питания:
* **Источник:** 2 x Li-Ion 14500.
* **Схема:** Последовательное соединение (2S).
* **Напряжение:** 7.4В (ном).
* **Емкость:** ~800 мАч (реальная, расчетная).

### Расчетное время работы (Repeater Mode):

| Режим | Потребление тока | Время работы (расчетное) |
|:---|:---:|:---:|
| **Активный (TX)** <br> *(Постоянная передача)* | ~150 мА | **~5 часов** |
| **Ожидание (RX)** <br> *(Прослушивание эфира)* | ~20 мА | **~40 часов** |
| **Глубокий сон** <br> *(Deep Sleep)* | ~0.5 мА | **> 3 месяцев** |

*\*Примечание: Расчет приведен для реальной емкости элементов типа 14500 (~800 мАч). Использование элементов 18650 (3000 мАч) увеличит время работы в 3-4 раза.*
...
* Скомпилировать APK и установить на телефон (требуется Android 8.0+)

2. ESP32 (Firmware)
(Код прошивки находится в папке /firmware)
* Установить Arduino IDE или PlatformIO.
* Установить библиотеки BluetoothSerial и LoRa (Sandeep Mistry).
* Настроить частоту (433E6 или 868E6) в define.
* Прошить устройства.
